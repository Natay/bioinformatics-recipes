...
if (length(params$tax_rank) != 0) {
  taxas <- params$tax_rank
} else {
  taxas <- "D,K,P,C,O,F,G,S"
}

tax_taxRanks <- unlist(strsplit(taxas, ","))

# tax_taxRanks <- c("D","K","P","C","O","F","G","S")
# tax_taxRanks <- c("D","O","F","G","S")
...
#  n="PT1"
  ...
  #    tabPanel("Sankey visualization",
        ...
        pavian:::build_sankey_network(my_report, taxRanks=tax_taxRanks, nodePadding=13, xScalingFactor=.9, nodeStrokeWidth=0, zoom = FALSE, colourScale = "sankey_colorscale", LinkGroup = "source_name")
